...
Core.PlayerFunctionOverrides.QSInventory = {
            ...
            for i = 1, #self.accounts do
                if self.accounts[i].name == account then
                    ...
                    if accounts[account] then
                        self.accounts[i].money = exports['qs-advancedinventory']:GetItemTotalAmount(self.source, account)
                    end
            ...
            if money >= 0 then
                ...
                if account then
                    ...
                    if accounts[accountName] and reason ~= 'dropped' then
                        exports['qs-advancedinventory']:SetInventoryItems(self.source, accountName, money)
                    end
                end
            end
        end
            ...
            if money > 0 then
                ...
                if account then
                    ...
                    if accounts[accountName] then
                        exports['qs-advancedinventory']:AddItem(self.source, accountName, money)
                    end
                end
            end
        end
        ...
        return function(accountName, money, reason)
            ...
            if money > 0 then
                ...
                if account then
                    ...
                    local accounts = exports['qs-advancedinventory']:GetAccounts()
                    if accounts[accountName] then
                        exports['qs-advancedinventory']:RemoveItem(self.source, accountName, money)
                    end
                end
            end
        end
    ...
    addInventoryItem = function(self)
        return function(name, count, metadata, slot)
            return exports['qs-advancedinventory']:AddItem(self.source, name, count or 1, slot, metadata)
        end
    end,
    removeInventoryItem = function(self)
        return function(name, count, metadata, slot)
            return exports['qs-advancedinventory']:RemoveItem(self.source, name, count or 1, slot, metadata)
        end
    end,
    setInventoryItem = function(self)
        return function(name, count, metadata)
            return exports['qs-advancedinventory']:SetInventoryItem(self.source, name, count, metadata)
        end
    end,
    canCarryItem = function(self)
        return function(name, count, metadata)
            return exports['qs-advancedinventory']:CanCarryItem(self.source, name, count)
        end
    end,
    ...
    hasItem = function(self)
        return function(name, metadata)
            return exports['qs-advancedinventory']:GetItemByName(self.source, name)
        end
    end,
